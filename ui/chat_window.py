...
from crypto import load_private_key, encrypt, decrypt
from messages import (
    get_all_users, get_public_key, save_message,
    update_message, delete_message_for_me, delete_message_for_everyone,
    fetch_messages, mark_messages_as_read, get_unread_count,
)
...
class ChatWindow(QWidget):
    ...
    def load_messages(self):
        ...
        self.last_signature = self._compute_signature(rows)
        for row in rows:
            self._add_bubble(row)
        self._scroll_to_bottom()

    def _add_bubble(self, row):
        ...
        bubble = MessageBubble(
            ...
            modifie=modifie_le is not None,
            lu=lu_bool,
    ...
    def _compute_signature(self, rows):
        if not rows:
            return (0, 0, "")
    ...
    def poll(self):
        ...
        if not self.selected_user:
            return
        rows = fetch_messages(self.username, self.selected_user)
        sig = self._compute_signature(rows)
